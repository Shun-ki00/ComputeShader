...
Microsoft::WRL::ComPtr<ID3D11DeviceContext> DeviceResources::CreateConfiguredDeferredContext()
{
    Microsoft::WRL::ComPtr<ID3D11DeviceContext> deferredContext;
    ThrowIfFailed(m_d3dDevice->CreateDeferredContext(0, &deferredContext));

    // --- 必要な状態をセット ---
    auto rtv = m_d3dRenderTargetView.Get();
    auto dsv = m_d3dDepthStencilView.Get();
    deferredContext->OMSetRenderTargets(1, &rtv, dsv);

    const auto& viewport = m_screenViewport;
